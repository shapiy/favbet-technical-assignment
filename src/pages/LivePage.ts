...
export class LivePage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // Navigation
    this.liveNavigationLink = page
      .locator('a[href*="/live"], a:has-text("Лайв"), a:has-text("Live")')
      .first();

    // Containers
    this.liveEventsContainer = page
      .locator('.live-events, [class*="live-list"], .events-container')
      .first();
    ...
    this.eventTitles = page.locator(
      '.event-title, .match-title, .team-names, [class*="event-name"]'
    );

    // UI elements
    this.loadingSpinner = page.locator('.loading, .spinner, [class*="loader"]');
    this.noEventsMessage = page.locator(
      '.no-events, .empty-message, :text("Немає подій"), :text("СПОРТ НЕ ЗНАЙДЕНО")'
    );
    this.sportFilters = page.locator('.sport-filter, [class*="sport-tab"], .category-filter');
    this.searchInput = page.locator(
      'input[type="search"], input[placeholder*="Пошук"], input[placeholder*="Search"]'
    );
  }
