...
export class LoginPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.rememberMeCheckbox = page.locator(
      'input[type="checkbox"][name="remember"], .remember-checkbox'
    );
    this.forgotPasswordLink = page.locator(
      'a:has-text("Забули пароль"), a:has-text("Forgot password")'
    );
  ...
  async login(emailOrPhone: string, password: string, rememberMe: boolean = false) {
    ...
    await Promise.race([
      ...
      this.errorMessage
        .waitFor({ state: 'visible', timeout: testData.timeouts.short })
        .catch(() => null),
    ]);
  }
