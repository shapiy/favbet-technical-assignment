...
test.describe('Favorites Management', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      ...
      const removedCount = await favoritesPage.removeAllFromFavorites();
      logger.info(`Cleared ${removedCount} favorite(s) before test`);
    } catch (error) {
      logger.warn('Failed to clear favorites before test:', error);
    }
  });
  ...
  test('should add items to favorites and manage them', async ({ page }) => {
    ...
    await test.step('Add several items to favorites', async () => {
      ...
      logger.info(`Found ${liveEventsCount} live events available`);
      ...
      logger.info(`Added ${addedFavorites.length} items to favorites:`, addedFavorites);
    });
    ...
    await test.step('Verify favorited items are present', async () => {
      ...
      logger.info(`Verified ${favoritesCount} favorites are present on the favorites page`);
    });
    ...
    await test.step('Remove one item from favorites', async () => {
      ...
      logger.info(`Removed item: ${removedItemTitle}`);
    });
