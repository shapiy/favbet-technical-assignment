  ...
  test('should change language and theme settings', async ({ page }) => {
    ...
    await test.step('Verify language change', async () => {
      ...
      if (currentUrl.includes('/en/')) {
        ...
        await expect(page.locator('[data-role="account_pageTitle_text"]')).toContainText(
          'Settings'
        );
    ...
    await test.step('Verify theme change', async () => {
      ...
      logger.info(
        `Theme info: isDarkTheme=${themeInfo.isDarkTheme}, backgroundColor=${themeInfo.backgroundColor}`
      );
