...
test.describe('Favbet API - Bonuses', () => {
  test('should authenticate and get user bonuses list', async ({ page }) => {
    ...
    const bonusResult = await page.evaluate(async () => {
      try {
        ...
        if (countResult.ok && countResult.data?.response?.response?.bonusCount > 0) {
          const wageringResponse = await fetch(
            '/service/crm_proxy/crm_api/gamegate/anybonus/wagering',
            {
              method: 'POST',
              headers: {
                Accept: 'application/json',
          ...
          wageringResult = {
            ...
            data: wageringResponse.ok
              ? await wageringResponse.json()
              : await wageringResponse.text(),
          };
        }
    ...
    // Check if we have an error or missing response
    if (bonusResult.error) {
      throw new Error(`API call failed: ${bonusResult.error}`);
    }

    expect(bonusResult.countResult).toBeDefined();
    const countResult = bonusResult.countResult!;

    // Log the API responses
    logger.info(`Bonus count API status: ${countResult.status} ${countResult.statusText}`);
    logger.info('Bonus count API response:', countResult.data);

    if (bonusResult.wageringResult) {
      logger.info(
        `Bonus wagering API status: ${bonusResult.wageringResult.status} ${bonusResult.wageringResult.statusText}`
      );
    ...
    expect(countResult.data).toBeDefined();
    ...
    const validateBonusData = (bonuses: BonusInfo[]) => {
      ...
      bonuses.forEach((bonus, index) => {
        ...
        if (
          bonus.wagering !== undefined &&
          (typeof bonus.wagering !== 'number' || bonus.wagering < 0)
        ) {
